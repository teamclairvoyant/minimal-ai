...
class SparkSinkWriter:
    ...
    async def db_writer(self) -> None:
        ...
        try:
            logger.info(
                "writing dataframe to - %s", self.current_task.sink_config["db_type"]
            )
            match self.current_task.sink_config["db_type"]:
                case "mysql":
                    _url = DB_MYSQL_URL.format(
                        host=self.current_task.sink_config["host"],
            ...
            _df.write.jdbc(
                url=_url,
                table=self.current_task.sink_config["table"],
    ...
    async def local_file_writer(self) -> None:
        ...
        try:
            logger.info(
                "writing dataframe to - %s", self.current_task.sink_config["file_type"]
            )
            _df: DataFrame = await DataframeUtils.get_df_from_alias(
                self.spark, self.current_task.upstream_tasks[0]
            )
        ...
        try:
            ...
            match self.current_task.sink_config["file_type"]:
                case "csv":
                    _df.write.csv(
                        path=self.current_task.sink_config["file_path"],
                        mode=self.current_task.sink_config["mode"],
                        sep=",",
                        header=True,
                    )
    ...
    async def bigquery_writer(self) -> None:
        ...
        try:
            ...
            _df.write.format("bigquery").option("writeMethod", "direct").mode(
                self.current_task.sink_config["mode"]
