...
async def add_pipeline(pipeline_config: PipelineModel) -> JSONResponse:
    ...
    try:
        ...
        pipeline = await PipelineService.create_pipeline(
            ...
            pipeline_config.executor_config,
...
async def update_pipeline(
    uuid: str, pipeline_config: PipelineUpdateModel
) -> JSONResponse:
    ...
    try:
        ...
        pipeline = await PipelineService.update_pipeline(uuid, pipeline_config)
        return JSONResponse(
            status_code=status.HTTP_201_CREATED, content={"pipeline": pipeline}
        )
    except MinimalETLException as excep:
...
async def get_schema_by_task(pipeline_uuid: str, task_uuid: str) -> JSONResponse:
    ...
    """
    try:
        logger.info("GET /pipeline/%s/task/%s/schema", pipeline_uuid, task_uuid)
...
async def get_sample_records(pipeline_uuid: str, task_uuid: str) -> JSONResponse:
    ...
    try:
        ...
        logger.error(
            "GET /sample_data - pipeline %s - task %s | %s",
            pipeline_uuid,
            task_uuid,
            excep.args,
        )
...
async def update_task(
    ...
    try:
        ...
        pipeline = await TaskService.update_task_by_config(
            pipeline_uuid, task_uuid, request
        )
...
@api_router.put("/pipeline/{pipeline_uuid}/task/{task_uuid}/addColumn")
async def add_column(
    pipeline_uuid: str, task_uuid: str, columns: TaskColumns
) -> JSONResponse:
    """endpoint to add column list to task

    Args:
        pipeline_uuid (str): uuid of pipeline
        task_uuid (str): uuid of task
        columns (TaskColumns): list of columns
    """
    try:
        logger.info("PUT /pipeline/%s/task/%s/addColumn", pipeline_uuid, task_uuid)
        pipeline = await TaskService.update_columns_by_task(
            pipeline_uuid, task_uuid, columns.columns
        )
        return JSONResponse(
            status_code=status.HTTP_200_OK, content={"pipeline": pipeline}
        )
...
async def execute_pipeline(pipeline_uuid: str) -> JSONResponse:
    ...
    try:
        ...
        async with transaction():
            pipeline = await PipelineService.execute_pipeline_by_uuid(pipeline_uuid)
        return JSONResponse(
            status_code=status.HTTP_200_OK, content={"pipeline": pipeline}
        )
