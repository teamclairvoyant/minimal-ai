...
function SinkConfig({ currNode, closeBar }) {
  ...
  return (
    <Stack spacing={2}>
      ...
      <Stack gap={2}>
        {sType === "file" && (
          <FileConfig closeBar={closeBar} currNode={currNode}></FileConfig>
        )}
        {sType === "rdbms" && (
          <RdbmsConfig closeBar={closeBar} currNode={currNode}></RdbmsConfig>
        )}
      </Stack>
    </Stack>
  );
...
const FileConfig = ({ closeBar, currNode }) => {
  ...
  async function handleSubmit() {
    ...
    const response = await backendApi.put(
      `/pipeline/${pipeline.uuid}/task/${task_id}`,
      payload
    );
  ...
  const areaChange = (event) => {
    ...
    if (
      event.target.value === "gcp_bucket" ||
      event.target.value === "aws_s3"
    ) {
  ...
  return (
    <>
      ...
      <ActionButtons closeBar={closeBar} handleSubmit={handleSubmit} />
    </>
  );
};
